...
class RangerToGprmc(object):
    ...
    def got_new_message(self, mdat_ranger):
        ...
        try:
            ...
            forward_message = "$GPRMC,{gps_time},A,{lat_deg:0>2df}{lat_decmin},{lat_dir},{lon_deg:0>3d}{lon_decmin},{lon_dir},{speed_kt},{heading},{gps_date},000.0,W\r\n".format(
                gps_time=gps_time,
                gps_date=gps_date,
                **mdat_ranger
            )
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='Decode CCL messages received by a modem and generate fake GPRMC messages to send out a serial port')
    #ap.add_argument("-l", "--logfile", help="Save log with specified name")
    ap.add_argument("-f", "--display-forward", action='store_true', help="Print the forwarded output to console rather than human-readable")
